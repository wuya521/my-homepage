...
1. 部署完成后，点击 **"编辑代码"**
2. 将项目中的 `worker.js` 文件内容完整复制到编辑器中
3. 点击右上角 **"保存并部署"**
4. **重要：记录下你的 Worker 域名**（格式：`https://your-worker-name.your-subdomain.workers.dev`）
...
3. 确认 `static/script.js` 中的 API_BASE 配置正确

### Q: 登录后台时提示"用户名或密码错误"？

**A:** 
1. 确认使用的是默认账号：`admin` / `admin123`
2. 如果已修改过密码但忘记了，需要重新部署 Worker 代码以重置
